...
docker run -d \
    ...
    -e BUNDLE_URL=https://ovidyou-meteord-tests.s3-us-west-2.amazonaws.com/meteord-test-bundle.tar.gz \
    -e REBUILD_NPM_MODULES=1 \
    -p 9090:80 \
    "ovidyou/meteord:node-${NODE_VERSION}-base"
...
if [[ $appContent != *"ovidyou/meteord:bin-build"* ]]; then
